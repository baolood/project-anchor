...
def run_gated_next_manual_low_frequency_production_order_request(
    body: Dict[str, Any],
    gate_config: Dict[str, Any] | None,
    credentials: Dict[str, str] | None,
    now_ts: int,
    *,
    ...
    gate_decision = next_manual_low_frequency_production_request_send_gate_decision(
        gate_config,
        now=now,
    )
    send_decision = next_manual_low_frequency_production_order_send_decision_response(
        body,
        gate_decision,
    )
    if send_decision.get("status") != "ready_for_exactly_one_send":
