#!/usr/bin/env python3
"""Validate non-executing production signing readiness evidence.

The validator only accepts non-secret status labels. It never reads secret
material and never performs signing, network calls, or production requests.
"""
...
DEFAULT_INPUT = ROOT / "config" / "production_signing_readiness.template.json"
REPORTS_DIR = ROOT / "reports"
JSON_OUT = REPORTS_DIR / "production_signing_readiness_validation.json"
MD_OUT = REPORTS_DIR / "production_signing_readiness_validation.md"
...
REQUIRED_FIELDS = [
    "PRODUCTION_SIGNING_READINESS_OPERATOR_FILLED",
    *STATUS_FIELDS,
    "AUTHORIZED_SECRET_VALUE_READ",
    "AUTHORIZED_SECRET_VALUE_DISCLOSURE",
    "AUTHORIZED_PRODUCTION_SIGNING_EXECUTION",
...
EXPECTED_LOCKED_FIELDS = {
    ...
    "AUTHORIZED_PRODUCTION_SIGNING_EXECUTION": "NO",
...
def validate(data: dict[str, Any], input_path: Path) -> dict[str, Any]:
    ...
    operator_filled = as_text(data.get("PRODUCTION_SIGNING_READINESS_OPERATOR_FILLED")).upper()
    if operator_filled != "YES":
        errors.append("PRODUCTION_SIGNING_READINESS_OPERATOR_FILLED:MUST_BE_YES")
    ...
    if final_verdict != "APPROVED_FOR_PRODUCTION_SIGNING_READINESS_ONLY":
        errors.append("FINAL_OPERATOR_VERDICT:NOT_APPROVED_FOR_PRODUCTION_SIGNING_READINESS_ONLY")
...
def markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Production Signing Readiness Validation
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print("PRODUCTION_SIGNING_READINESS_VALIDATION=FAIL")
    ...
    print("[Production Signing Readiness Validation]")
