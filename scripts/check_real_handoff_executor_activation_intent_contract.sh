#!/usr/bin/env bash
# Verifies the bounded intent contract for a future real handoff executor
# activation without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_executor_activation_intent_contract.sh

Checks a fixture matrix for the future real handoff executor-activation intent
contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- executor-activation preflight contract must already be green
- intent must remain review_only
...
- non-green executor-activation preflight
- intent that requests activation_now
...
ALLOWED_KEYS = {
    "intent_id",
    "reviewed_state",
    "executor_activation_preflight_status",
    "activation_intent",
...
def validate(payload):
    ...
    if payload.get("executor_activation_preflight_status") != "green":
        reasons.append("executor_activation_preflight_not_green")
    if payload.get("activation_intent") != "review_only":
        reasons.append("activation_intent_not_review_only")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_EXECUTOR_ACTIVATION_INTENT_CONTRACT_CHECK FAIL: "
...
base = {
    "intent_id": "real-handoff-executor-activation-intent-20260526-001",
    "reviewed_state": "ready_for_real_review",
    "executor_activation_preflight_status": "green",
    ...
    "ticket_ref": "RHEAI-001",
    "notes": "bounded executor activation intent contract",
}

covered = [
    assert_case("minimal_valid_executor_activation_intent", dict(base), True),
    assert_case("preflight_not_green", dict(base, executor_activation_preflight_status="blocked"), False),
    assert_case("activation_now_requested", dict(base, activation_intent="activation_now"), False),
...
print(
    "REAL_HANDOFF_EXECUTOR_ACTIVATION_INTENT_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
