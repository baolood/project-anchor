...
HTTP=$(curl "${CURL_FLAGS[@]}" -o /tmp/ops_worker.json -w "%{http_code}" "$CONSOLE_URL/api/ops/worker")
...
HTTP_ON=$(curl "${CURL_FLAGS[@]}" -o /dev/null -w "%{http_code}" -X POST \
...
OBS_ON=$(curl "${CURL_FLAGS[@]}" "$CONSOLE_URL/api/ops/worker" | grep -c '"kill_switch_enabled":true' || true)
echo "OBSERVED_ON=$([ "$OBS_ON" -gt 0 ] && echo YES || echo NO)" >> "$OUT"

HTTP_OFF=$(curl "${CURL_FLAGS[@]}" -o /dev/null -w "%{http_code}" -X POST \
  -H "content-type: application/json" \
  -d '{"enabled":false}' \
  "$CONSOLE_URL/api/ops/kill-switch")
...
OBS_OFF=$(curl "${CURL_FLAGS[@]}" "$CONSOLE_URL/api/ops/worker" | grep -c '"kill_switch_enabled":false' || true)
