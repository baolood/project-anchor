#!/usr/bin/env bash
# Verifies the bounded preflight contract for a future real handoff executor
# activation without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_executor_activation_preflight_contract.sh

Checks a fixture matrix for the future real handoff executor-activation
preflight contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- execution-request approval gate contract must already be green
...
- non-green execution-request approval gate
...
ALLOWED_KEYS = {
    "preflight_id",
    "reviewed_state",
    "execution_request_approval_gate_status",
...
def validate(payload):
    ...
    if payload.get("execution_request_approval_gate_status") != "green":
        reasons.append("execution_request_approval_gate_not_green")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_EXECUTOR_ACTIVATION_PREFLIGHT_CONTRACT_CHECK FAIL: "
...
base = {
    "preflight_id": "real-handoff-executor-activation-preflight-20260526-001",
    "reviewed_state": "ready_for_real_review",
    "execution_request_approval_gate_status": "green",
    ...
    "ticket_ref": "RHEAP-001",
    "notes": "bounded executor activation preflight contract",
}

covered = [
    assert_case("minimal_valid_executor_activation_preflight", dict(base), True),
    assert_case("approval_gate_not_green", dict(base, execution_request_approval_gate_status="blocked"), False),
...
print(
    "REAL_HANDOFF_EXECUTOR_ACTIVATION_PREFLIGHT_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
