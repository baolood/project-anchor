#!/usr/bin/env bash
# Verifies the bounded launch-intent contract for a future real handoff
# executor launch without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_executor_launch_intent_contract.sh

Checks a fixture matrix for the future real handoff executor-launch-intent
contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- executor-launch-packet contract must already be green
- launch intent must remain review_only
...
- non-green executor-launch-packet contract
- launch intent that requests execute_now
...
def validate(payload):
    ...
    if payload.get("executor_launch_packet_status") != "green":
        reasons.append("executor_launch_packet_not_green")
    if payload.get("launch_intent_mode") != "review_only":
        reasons.append("launch_intent_mode_not_review_only")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_EXECUTOR_LAUNCH_INTENT_CONTRACT_CHECK FAIL: "
...
base = {
    "launch_intent_id": "real-handoff-executor-launch-intent-20260526-001",
    "reviewed_state": "ready_for_real_review",
    "executor_launch_packet_status": "green",
    "launch_intent_mode": "review_only",
    ...
    "ticket_ref": "RHELI-001",
    "notes": "bounded executor launch intent contract",
}

covered = [
    assert_case("minimal_valid_executor_launch_intent", dict(base), True),
    assert_case("launch_packet_not_green", dict(base, executor_launch_packet_status="blocked"), False),
    assert_case("execute_now_requested", dict(base, launch_intent_mode="execute_now"), False),
...
print(
    "REAL_HANDOFF_EXECUTOR_LAUNCH_INTENT_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
