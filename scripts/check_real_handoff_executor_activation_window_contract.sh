#!/usr/bin/env bash
# Verifies the bounded window contract for a future real handoff executor
# activation without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_executor_activation_window_contract.sh

Checks a fixture matrix for the future real handoff executor-activation window
contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- executor-activation intent contract must already be green
- activation window must remain review_only and scheduled, not immediate
- expected executor posture must remain mock / real_enable=0
- approved base URL and placeholder-only credential state may appear

Rejected contract themes:
- non-green executor-activation intent
- immediate activation window
...
ALLOWED_KEYS = {
    "window_id",
    "reviewed_state",
    "executor_activation_intent_status",
    "activation_window_mode",
    "window_label",
...
def validate(payload):
    ...
    if payload.get("executor_activation_intent_status") != "green":
        reasons.append("executor_activation_intent_not_green")
    if payload.get("activation_window_mode") != "scheduled_review_only":
        reasons.append("activation_window_mode_not_scheduled_review_only")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_EXECUTOR_ACTIVATION_WINDOW_CONTRACT_CHECK FAIL: "
...
base = {
    "window_id": "real-handoff-executor-activation-window-20260526-001",
    "reviewed_state": "ready_for_real_review",
    "executor_activation_intent_status": "green",
    "activation_window_mode": "scheduled_review_only",
    "window_label": "first-controlled-send-window",
    ...
    "ticket_ref": "RHEAW-001",
    "notes": "bounded executor activation window contract",
}

covered = [
    assert_case("minimal_valid_executor_activation_window", dict(base), True),
    assert_case("intent_not_green", dict(base, executor_activation_intent_status="blocked"), False),
    assert_case("activation_window_immediate", dict(base, activation_window_mode="immediate"), False),
...
print(
    "REAL_HANDOFF_EXECUTOR_ACTIVATION_WINDOW_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
