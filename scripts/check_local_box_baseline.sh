...
# First-controlled-send candidate-window record must stay bounded as a
# docs-only bridge before any future real window opening can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_candidate_window_record.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send candidate window record check failed" >&2
  exit 1
fi

# First-controlled-send candidate-window closeout must stay coherent as a
# docs-only closeout surface before any future real window opening can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_candidate_window_closeout.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send candidate window closeout check failed" >&2
  exit 1
fi
...
# First-controlled-send window-open closeout must stay coherent as a
# docs-only opening posture before any future real window opening can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_window_open_closeout.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send window open closeout check failed" >&2
  exit 1
fi

# First-controlled-send window-open record must stay bounded as a docs-only
# operational record before any future send attempt can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_window_open_record.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send window open record check failed" >&2
  exit 1
fi

# First-controlled-send window-open record closeout must stay coherent as a
# docs-only transition posture before any future send attempt can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_window_open_record_closeout.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send window open record closeout check failed" >&2
  exit 1
fi
...
# First-controlled-send runtime-verification closeout must stay coherent as a
# docs-only verification posture before any future send attempt can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_runtime_verification_closeout.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send runtime verification closeout check failed" >&2
  exit 1
fi

# First-controlled-send attempt record must stay bounded as a docs-only
# operational record before any future final review can be considered.
if ! "${ROOT}/scripts/check_real_testnet_first_controlled_send_attempt_record.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: first controlled send attempt record check failed" >&2
  exit 1
fi
...
# Hardened ORDER:testnet one-shot invocation must stay fail-closed before any
# future operator-authorized window can execute it.
if ! bash "${ROOT}/scripts/check_hardened_order_testnet_one_shot_invocation.sh"; then
  echo "LOCAL_BOX_BASELINE_CHECK FAIL: hardened ORDER:testnet one-shot invocation check failed" >&2
  exit 1
fi
