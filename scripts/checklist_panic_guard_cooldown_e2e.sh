...
echo "=============================="
trigger_resp="$(curl "${CURL_FLAGS[@]}" -X POST -H "Content-Type: application/json" -w "\n%{http_code}" "$CONSOLE_PRECHECK/api/proxy/ops/panic_guard/trigger")"
...
echo "=============================="
trigger2_resp="$(curl "${CURL_FLAGS[@]}" -X POST -H "Content-Type: application/json" -w "\n%{http_code}" "$CONSOLE_PRECHECK/api/proxy/ops/panic_guard/trigger")"
...
if [ "$trigger2_code" != "409" ]; then
  ...
  echo "PASS_OR_FAIL=$PASS_OR_FAIL"
  curl "${CURL_FLAGS[@]}" -X POST -H "Content-Type: application/json" "$CONSOLE_PRECHECK/api/proxy/ops/panic_guard/reset" >/dev/null || true
  exit 1
fi
...
echo "=============================="
reset_resp="$(curl "${CURL_FLAGS[@]}" -X POST -H "Content-Type: application/json" -w "\n%{http_code}" "$CONSOLE_PRECHECK/api/proxy/ops/panic_guard/reset")"
