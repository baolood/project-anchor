#!/usr/bin/env bash
# Verifies the bounded contract for a future real handoff opening bundle
# without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_opening_bundle_contract.sh

Checks a fixture matrix for the future real handoff opening-bundle contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- opening-prereq line must already be green
- expected executor posture must remain mock / real_enable=0
- bundle references, reviewer labels, and credential slot names may appear

Rejected contract themes:
- non-green opening-prereq line
...
def validate(payload):
    ...
    if payload.get("opening_prereq_line_status") != "green":
        reasons.append("opening_prereq_line_not_green")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_OPENING_BUNDLE_CONTRACT_CHECK FAIL: "
...
covered = [
    assert_case("minimal_valid_opening_bundle", dict(base), True),
    assert_case("reviewed_state_not_ready", dict(base, reviewed_state="synthetic_only"), False),
    assert_case("opening_prereq_line_not_green", dict(base, opening_prereq_line_status="blocked"), False),
...
print(
    "REAL_HANDOFF_OPENING_BUNDLE_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
