#!/usr/bin/env bash
# Verifies the bounded launch-window contract for a future real handoff
# executor launch without injecting credentials or changing runtime.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/check_real_handoff_executor_launch_window_contract.sh

Checks a fixture matrix for the future real handoff executor-launch-window
contract.

Allowed contract themes:
- reviewed state must already be ready_for_real_review
- executor-launch-intent contract must already be green
- launch window must remain scheduled_review_only
...
- non-green executor-launch-intent contract
...
def validate(payload):
    ...
    if payload.get("executor_launch_intent_status") != "green":
        reasons.append("executor_launch_intent_not_green")
    if payload.get("launch_window_mode") != "scheduled_review_only":
        reasons.append("launch_window_mode_not_scheduled_review_only")
...
def assert_case(name, payload, should_pass):
    ...
    if passed != should_pass:
        raise SystemExit(
            "REAL_HANDOFF_EXECUTOR_LAUNCH_WINDOW_CONTRACT_CHECK FAIL: "
...
base = {
    "launch_window_id": "real-handoff-executor-launch-window-20260526-001",
    "reviewed_state": "ready_for_real_review",
    "executor_launch_intent_status": "green",
    "launch_window_mode": "scheduled_review_only",
    "window_label": "first-controlled-send-launch-window",
    ...
    "ticket_ref": "RHELW-001",
    "notes": "bounded executor launch window contract",
}

covered = [
    assert_case("minimal_valid_executor_launch_window", dict(base), True),
    assert_case("launch_intent_not_green", dict(base, executor_launch_intent_status="blocked"), False),
    assert_case("launch_window_immediate", dict(base, launch_window_mode="immediate"), False),
...
print(
    "REAL_HANDOFF_EXECUTOR_LAUNCH_WINDOW_CONTRACT_CHECK PASS: fixture matrix intact for "
    + ",".join(covered)
)
PY
