...
- Quick fail triage helper: `./scripts/check_local_box_ci_runs.sh --branch <your-branch> --failed-only` (**`--branch`** is passed through to **`gh`**; scope per-branch before **`--limit`** truncates the list). For pipeline gating, add `--latest-only --fail-on-failed`.
