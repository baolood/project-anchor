...
- **Go-live tooling:** execution checklist, daily status reporter, `artifacts/go-live/` evidence layout; CI smoke + baseline require the tracked onboarding README. **`go_live_status_report.sh --out`** must target a file path (directories are rejected).
