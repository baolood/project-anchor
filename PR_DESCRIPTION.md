...
- **CI triage helper:** `check_local_box_ci_runs.sh` forwards **`--branch`** to **`gh run list`**; **`--failed-only`** narrows to completed non-success, non-cancelled runs (mutually exclusive with **`--cancelled-only`**); **`--latest-only --fail-on-failed`** (alias: **`--fail-on-non-success`**) gates branch health, and **`--fail-on-incomplete`** can enforce that runs have settled (shortcut: **`--gate-strict`**).
