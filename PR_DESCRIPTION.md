...
- **Go-live tooling:** execution checklist, daily status reporter, `artifacts/go-live/` evidence layout; CI smoke + baseline require the tracked onboarding README. **`local-box-baseline`** runs the reporter **stdout-only**; **`go_live_status_report.sh --out`** is for **local** standup files and must target a file path (directories are rejected).
