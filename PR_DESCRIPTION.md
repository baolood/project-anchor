...
- **CI triage helper:** `check_local_box_ci_runs.sh` forwards **`--branch`** to **`gh run list`**; **`--failed-only`** narrows to completed non-success, non-cancelled runs (mutually exclusive with **`--cancelled-only`**); **`--latest-only --fail-on-failed`** (alias: **`--fail-on-non-success`**) gates branch health, **`--fail-on-incomplete`** enforces settled runs, and **`--fail-on-empty`** fails when filters return no rows (shortcut: **`--gate-strict`** requires **`--branch`**).
