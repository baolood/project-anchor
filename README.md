...
Workflow hardening in the YAML: **`actions/checkout`** uses **`persist-credentials: false`**; job **`check`** sets **`PYTHONPATH=.`** for all steps (the inline Python smokes import **`local_box`** from repo root); **`actions/setup-python`** enables **pip** dependency caching keyed off **`requirements.txt`** plus **`requirements.in`** (warm runs reinstall faster; **`requirements.in`** changes invalidate stale wheels).
...
3. Job **`check`** runs **`./scripts/check_local_box_baseline.sh`** (`--help` explains scope; required paths include **`local_box`**, **`shared`**, **`risk_engine`**, **`artifacts/go-live/README.md`**, **`.github/pull_request_template.md`**, plus checklist `curl` guardrails)
