...
Workflow hardening in the YAML: **`actions/checkout`** uses **`persist-credentials: false`**; job **`check`** sets **`PYTHONPATH=.`** for all steps (the inline Python smokes import **`local_box`** from repo root); **`actions/setup-python`** enables **pip** dependency caching keyed off **`requirements.txt`** (warm runs reinstall faster).
