...
- **Go-live status reporter:** [`scripts/go_live_status_report.sh`](scripts/go_live_status_report.sh) (standup evidence: `mkdir -p artifacts/go-live` then `--out artifacts/go-live/go_live_daily_status_$(date +%F).out` — see [`docs/GO_LIVE_CHECKLIST.md`](docs/GO_LIVE_CHECKLIST.md) §7 and [`artifacts/go-live/README.md`](artifacts/go-live/README.md); `/tmp/...` is only for ad-hoc checks; `artifacts/go-live/*.out` is gitignored)
