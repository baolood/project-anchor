...
- **`anchor-backend/`** subtree：**`git ls-files --others --exclude-standard` 语境下仍为未跟踪的草稿文件为 **2** 个（见 §2.2）；另 **1** 份 **`DOMAIN_COMMAND_VALIDATION_DEV_USAGE_V1.md`** **已升格跟踪**（见 §2.1）。
- **当前**：不删除、不移动 §2.2 **仍未跟踪**路径，亦 **不以未立项方式直接并入主线**。
- **§2.2 仍存在未跟踪**的两份：统一状态 **`pending_decision`**。
...
### 2.1 已升格入库（跟踪；不再出现于 `git ls-files --others`）
...
截至本条记录：**§2.1** 已跟踪；**§2.2 两份**仍为 **`pending_decision`** **未跟踪**。
...
- **删除或移动**上述 **§2.2 pending** 草稿路径：**禁止**在未授权任务下执行。
...
- **`git status`**：§2.2 **pending** 路径仍应保持 **未跟踪**形态。（§2.1 已升格项应 **不出现** **`??`**。）
