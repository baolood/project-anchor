...
> **留痕说明**：§1–§6 冻结 **首次** run（失败，见下文）；§9 冻结**成功** run（含 `local_box` 入树后及后续文档推送）。**不得**用本地手跑替代远程 run。**失败 run：** **`24642983527`**；**成功 run（入树）：** **`24643041767`**；**成功 run（证据文档）：** **`24643098559`**；**成功 run（`main` merge 后）：** **`24643589825`**；**成功 run（`main` 文档推送 `9c6805a`，Run #7）：** **`24643718594`**；**成功 run（`main` 追加 §9.3 / `2f7be15`，Run #13）：** **`24644103202`**；**成功 run（`main` 同步 STATUS+§9.4 / `7758b1f`，Run #14）：** **`24644350931`**；**成功 run（`main` 记录 Run #14 / `abe5fe9`，Run #15）：** **`25312184296`**；**成功 run（`main` 记录 Run #15 / `9a27a36`，Run #16）：** **`25315241628`**（`2026-04-20`）。
...
### 9.6 `main` 上回填 Run #14 证据后的运行（网页 Run #15）

- **分支：** **`main`**。
- **提交：** **`abe5fe9`**（`docs: record Run #14 for baseline STATUS sync on main`）。
- **Workflow run：** **[`25312184296`](https://github.com/baolood/project-anchor/actions/runs/25312184296)** — GitHub UI **run_number**：**`15`** — `conclusion`：**`success`**（与 `GET /repos/baolood/project-anchor/actions/runs/25312184296` 一致）。
- **stdout** 与同脚本 PASS 语义一致（见 §9.1 两行摘引）。

### 9.7 `main` 上回填 Run #15 证据后的运行（网页 Run #16）

- **分支：** **`main`**。
- **提交：** **`9a27a36`**（`docs: record Run #15 after Run #14 evidence on main`）。
- **Workflow run：** **[`25315241628`](https://github.com/baolood/project-anchor/actions/runs/25315241628)** — GitHub UI **run_number**：**`16`** — `conclusion`：**`success`**（与 `GET /repos/baolood/project-anchor/actions/runs/25315241628` 一致）。
- **stdout** 与同脚本 PASS 语义一致（见 §9.1 两行摘引）。

### 9.8 维护口径（避免无意义追 run）

- **§9.3 起**为「`main` 文书迭代触发的 PASS 链」快照；完整列表可随时用 **`GET /repos/baolood/project-anchor/actions/workflows/local-box-baseline.yml/runs?branch=main`**（或 Actions 网页）核对。
- **后续**仅以纯文档补齐 run id **并非**必选：在 **baseline 脚本、workflow 语义或证据口径** 发生结构性变更时，再增补 §9.x 即可。
