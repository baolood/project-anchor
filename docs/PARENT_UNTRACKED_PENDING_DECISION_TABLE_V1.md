...
- 父仓 **未跟踪路径**曾以 **初版 13 项**快照登记（见 §2 附：**历史对齐**）；现阶段 **`git ls-files --others --exclude-standard` 口径下仍 pending 的未跟踪路径为 7 个**（见 §2 **当前权威清单**）。
- **对这 7 个路径**：在未新唯一立项前，不执行删除，不归档实体，不并入主线，不对其做未授权 `git add`。
- **这 7 项**的统一状态：**`pending_decision`**。
...
### 2.1 仍为未跟踪（计数 **7**；与 `git ls-files --others --exclude-standard` 一致，`anchor.db` 等已忽略项不在此列）

1. `anchor-backend/scripts/insert_order_command.py`
2. `anchor-backend/worker/WORKER_BOUNDARY_RULES.md`
3. `execution_service/executor.py`
4. `execution_service/server.py`
5. `execution_service/verifier.py`
6. `risk_engine/client.py`
7. `shared/schemas.py`
...
| 上表 §2.1 所列 **7** 条 | 仍为 `pending_decision` **未跟踪** |
...
- **提交核对**：在仅暂存本文档的理想情况下，`git diff --cached --name-only` 应 **只列出**本文档路径；**§2.1 所列 7 个未跟踪路径不参与暂存**，仍应保持 **`git status`** 的 **`??`** 形态（与 §2.2 已升格/已归档路径**不**矛盾）。
- **禁止**：新增、删除、移动 **§2.1 所列 7 个路径**的任何实体文件或目录层级（除非下一轮单独立项明确授权）。
...
与 **`git ls-files --others --exclude-standard`** 口径一致（**2026-04 台账同步**）：
