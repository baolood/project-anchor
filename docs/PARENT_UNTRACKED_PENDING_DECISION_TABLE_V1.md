...
| `risk_engine/client.py` | **已升格跟踪** **`risk_engine/client.py`**（**`183a011`** **`fix(risk_engine): restore tracked client for local_box imports`**；误双源 **`docs/archive/risk_engine_draft/client.py`** 于 **`8164c0a`** 从树移除；见 **`RISK_ENGINE`** **§10**） |
...
- **`risk_engine/client.py`** 现为 **跟踪** **`risk_engine/`** 包内 **SSOT**（**`local_box`** 硬依赖 **`from risk_engine.client`**）；误入库 **`docs/archive/risk_engine_draft/client.py`** 已于 **`8164c0a`** 从 **Git 树**移除（**`183a011`** 恢复顶层 client），见 **`RISK_ENGINE_PENDING_DECISION_RECORD_V1.md`** **§10**。
